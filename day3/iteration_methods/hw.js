...
const newArr = recipes.filter((elem) => {
    const { rating } = elem;
    if (rating > 4.5) return true;
    else return false;
});

const finalArr = newArr.map((elem) => {
    const { name, cuisine } = elem;
    return {
        name,
        cuisine,
